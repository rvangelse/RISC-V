...
#Si n = 1, entonces, rec(n) = 1
#Si n = 2, entonces, rec(n) = 1 
#Por otro lado, si n > 2, rec(n) = ~(rec (n-1)) xor (rec(n-2))
...
.text
    ...
    rec:
        ...
        li t1, 3 #Cargo en t1, el valor inmediato 3, que usare para filtrar los casos base
        blt a0, t1, casosBase #Realizo una comparacion entre a0 y 3 (t1). Si a0 es menor que t1, estoy ante un caso base y salto a la etiqueta "casosBase"
        ...
        sw a0, 8(sp) #En el tercer espacio de la pila guardo el valor de a0, luego del segundo llamado a la funcion rec, a0 = rec(n-1)
        ...
        jal ra, rec #Realizo un tecer llamado a la funcion rec de forma recursiva, con a0 = (n - 2) como argumento
        lw t0, 8(sp) #Cargo en t0, el valor que guarde en el tercer espacio de la pila, es decit, t0 = rec (n-1)
        xori t0, t0, -1 #Realizo un xor entre t0(rec(n-1)) y 1, es decir, niego a t0. t0 = ~(rec(n-1))
        xor a0, a0, t0 #Realizo un xor entre t0 (~(rec(n-1))) y a0 (rec(n-2)) y cargo el resultado en a0, a0 = ~rec(n-1) xor rec(n-2) 
    ...
    casosBase:
        ...
        j epilogo #Salto al epilogo para retornar
...
#Al finalizar la ejecucion del programa, el valor cargado en a0 deberia ser el resultado de rec(3) = -1 (FFFFFFFF, en hexadecimal)